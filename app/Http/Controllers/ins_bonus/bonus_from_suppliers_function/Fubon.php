...
class Fubon extends Controller
{
    public static function bonusOri($file, $doc_name, $period, $supplier)
    {
        ...
        foreach (explode("\n", $file) as $file_key => $file_value) {
            $strlen_detection = \strlen($file_value);
            switch ($strlen_detection) {
                case ($strlen_detection > 380):
                    if ($strlen_detection !== 0) {
                        $array = self::formatOverStrlen(
                            $file_value,
                            $array,
                            $doc_name,
                            $period,
    ...
    private static function formatOverStrlen($file_value, $array, $doc_name, $period, $supplier)
    {
        echo vaR_dump($file_value);
        exit;
